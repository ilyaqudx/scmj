package freedom.scmj.common.var3d.net.freefont;
...
public class FreeBitmapFont extends BitmapFont {
	 ...
	 * 设置一个字符为自定义图片
	 */
	public FreeBitmapFont appendEmoji(String txt, Pixmap pixmap, int offsetX,
			int offsetY) {
		Pixmap pixmap2 = new Pixmap(paint.getTextSize(), paint.getTextSize(),
				Format.RGBA8888);
		pixmap2.drawPixmap(pixmap, offsetX, offsetY);
		pixmap.dispose();
		pixmap = null;
		char c = txt.charAt(0);
		putGlyph(c, pixmap2);
	...
	private void create(String characters, boolean haveMinPageSize) {
		...
		for (char c : characters.toCharArray()) {
			boolean isNewChar = charSet.add(c + "");
			if (isNewChar)
				cs.add(c + "");
		}
		// 根据字符参数和字符数量计算一个最小尺寸
